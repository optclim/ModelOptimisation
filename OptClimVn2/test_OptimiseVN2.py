...
from OptClimVn2.OptimiseVN2 import calcErr, iterGaussNewtonLinesearch, randSelect, \
    GaussNewtonLinesearch, GNjacobian, setOptimiseDefaults, runJacobian
...
class TestcalcErr(unittest.TestCase):
    ...
    # update strategy. run iterGaussNewtonLineSearch so that get info back
    # compare that against _ref solutions for linesearch generation and next iter.
...
class TestGNLS(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # check optfunction works as expected..
        testObs = self.optFunction(self.orig_param)
        if len(testObs) != self.nobs:
            raise ValueError("No obs not as expected")
        ...
        # compute values and check as expected
        testObs = self.optFunction(self.param_value)
        expectShape = (self.nparam + 1, self.nobs)
        if testObs.shape != expectShape:
            raise ValueError("No obs as array not as expected got " + repr(testObs.shape) +
                             " expected " + repr(expectShape))
    ...
    def run_gauss_newton(self, ref=False, trace=False):
        """ Run gaussnewton (or ref_doGaussNewton) and package results up in a dictionary """

        # TODO produce sensible function which generates expected values.
        # suspect better to put it in setup then generate values for ref code...
        # from it.
        if ref:
            ...
            study = setOptimiseDefaults(self.studyJSON)
            status, y, err, err_constraint, info = \
                iterGaussNewtonLinesearch(optfunction,
                                          self.orig_param, self.step, self.param_range, study.get('alphas'), self.obs,
                                          covTotal=self.cov.copy(), covInt=self.LScov_iv.copy(),
                                          nrandom=study.get('nrandom'), rngSeed=0,
                                          covar_cond=study.get('covar_cond'),
                                          hessian_regularize=(study.get('reg_crit_covar'), study.get('reg_pow_range')),
                                          minImprovement=study.get('minImprovement'),
                                          chisqCrit=(study.get('prob_int'), study.get('prob_obs')),
                                          verbose=trace)
    ...
    def run_LineSearch(self, ref=False, trace=False):
        ...
        if ref:
            ...
            status, err, err_constraint, NewParam, index, bestParam, info = iterGaussNewtonLinesearch(
    ...
    def _test_constraint_error(self):
    ...
    def _test_transformed(self):
        ...
        r->Mr, J->MJ and C-> MCM^T as long as M is invertible that results don't change. """

        # Generate  a symmetric matrix (which should be invertible)
    ...
    def _test_regularise(self):
        ...
        parameter differences and work as before"""
    ...
    def _test_random_comp_ref(self):
    ...
    def _test_LS_rand_std_ref(self):
    ...
    def _test_GaussNewtonLinesearch(self):
        ...
        best, status, info = GaussNewtonLinesearch(fn, startParam, paramRange, paramStep, tgt, optimise,
                                                   cov=cov, cov_iv=cov_iv, trace=True)
        self.assertEqual(status, 'Converged')
        nptest.assert_allclose(fn(best), tgt, atol=1e-3)  # reached the target
