...
import StudyConfig
import config  # get in configuration information.,
...
class testRunSubmit(unittest.TestCase):
    ...
    def setUp(self):
        ...
        refDir = pathlib.Path('test_in')

        if testDir.exists():  # remove directory if it exists
            shutil.rmtree(testDir, onerror=optClimLib.errorRemoveReadonly)
        self.dirPath = testDir

        refDirPath = refDir / 'start'
        self.refPath = refDirPath  # where the default reference configuration lives.
        self.testDir = testDir  # where all output goes.
        self.refDataPath = refDir  # where we can get some data from
        # now do stuff.
        ...
        # reset the reference dir.
        configData.referenceConfig(refDirPath)

        self.config = copy.deepcopy(configData)  # make a COPY of the config.
        self.Model = ModelSimulation.ModelSimulation  # what model to make

        # and finally generate a runSubmit object with some data in it.
        configData = self.config
        begin = configData.beginParam()
        testDir = self.testDir
        parameters = configData.fixedParams()
        parameters.update(refDir=self.refPath)
        dUP = dict()
        ...
        for count, dir_dirname in enumerate(['zz001', 'zz002']):
            ...
            outFile = createDir / configData.postProcessOutput()

            shutil.copy(self.refDataPath / '01_GN/h0101/observables.nc',
        ...
        self.rSubmit = runSubmit.runSubmit(configData, self.Model,
    ...
    def test_runDFOLS(self):
        ...
        filename = '$OPTCLIMTOP/Configurations/dfols14param.json'
        configData = StudyConfig.readConfig(filename)
